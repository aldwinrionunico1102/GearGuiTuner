...
// SETUP --------------------------------------------------------
void setup() {
  Serial.begin(115200);           // Used for testing and monitoring
...
double getOfficialFrequency(int stringIndex) {            // Get the most frequent frequency detected for the 3-second time
  ...
  if (validCount == 0) {
    Serial.println("No valid frequency detected — returning 0 Hz");
  ...
  Serial.print("\nMost frequent valid frequency: ");
  Serial.print(mostFrequent);
  Serial.println(" Hz\n");
  return mostFrequent;
}
...
void adjustTuning(int stringIndex, double diffHz) {       // Adjust the tuning pegs via stepper motors
  ...
  Serial.print(diffHz);
  Serial.print(" | Moving ");
  Serial.print(tighten ? "tighten " : "loosen ");
  Serial.print(stepDir > 0 ? "(CW)" : "(CCW)");
  Serial.print(" by ");
